...
class RRTPlanner(object):
    ...
    def steer_towards(self, s_nearest, s_rand, max_radius):
        ...
        if s_nearest.euclidean_distance(s_rand) <= max_radius:
            ...
            # dist_between = int(s_nearest.euclidean_distance(s_rand))
            # max_r = int(max_radius)
            # rand_x = s_rand.x
            # rand_y = s_rand.y
            # new_x =  int(rand_x * (max_radius / dist_between))
            # new_y = int(rand_y * (max_radius / dist_between))
            # s_new = State(new_x, new_y, s_nearest)
            x1, y1 = s_nearest.x, s_nearest.y
            x2, y2 = s_rand.x, s_rand.y
            theta = np.arctan2(y2-y1, x2-x1)
            new_x = x1 + max_radius * cos(theta)
            new_y = y1 + max_radius * sin(theta)
            s_new = State(int(new_x), int(new_y), s_nearest)
        return s_new
        ## Below are what's in the starter code...
        # x = 0
        # y = 0
    ...
    def path_is_obstacle_free(self, s_from, s_to):
        ...
        for i in range(max_checks):
            ...
            # inter_val = float(i)/max_checks * s_from.euclidean_distance(s_to)
            step_size = float(i)/max_checks
            x1, y1 = s_from.x, s_from.y
            x2, y2 = s_to.x, s_to.y
            theta = np.arctan2(y2-y1, x2-x1)
            x = x2 + step_size * np.cos(theta)
            y = y2 + step_size * np.sin(theta)
            interpolated_state = State(int(x), int(y), None)
    ...
    def plan(self, start_state, dest_state, max_num_steps, max_steering_radius, dest_reached_radius):
        ...
        for step in range(max_num_steps):
            ...
            # print("The type of s_nearest: ", type(s_nearest))
            # print("The type of s_new: ", type(s_new))
